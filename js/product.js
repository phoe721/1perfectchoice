$(document).ready(function() {
    ...
    // Helper for populating and updating fields
    function fillItemFields(data, input) {
        ...
        // Prevent accumulating change/focus/blur/click handlers
        $('input, textarea').off('change.updateField').on('change.updateField', function() {
        ...
        // Image update handler, always current
        $('#product_img').off('click.changeImage').on('click.changeImage', function() {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    if (resp.error == 'SKU not found!') {
                        $('#error').text(resp.error);
                    } else {
                        $('#product_img').attr('src', resp.img_url).prop('alt', resp.sku);
                    }
                }
            });
        });
