...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["sku"]) && isset($_POST["field"]) && isset($_POST["value"])) { 
	...
	if ($field == "upc") {
		...
		if ($packages->check_exist($code, $item_no)) {
			...
			$result = $packages->insert_dimensions($code, $item_no, $dimension[0], $dimension[1], $dimension[2], $dimension[3], $dimension[4], $dimension[5], $dimension[6], $dimension[7], $dimension[8], $dimension[9], $dimension[10], $dimension[11], $dimension[12], $dimension[13], $dimension[14], $dimension[15], $dimension[16], $dimension[17], $dimension[18], $dimension[19], $dimension[20], $dimension[21], $dimension[22], $dimension[23], $dimension[24], $dimension[25], $dimension[26], $dimension[27], $dimension[28], $dimension[29]); 
		}
		...
		if ($packages->check_exist($code, $item_no)) {
			...
			$result = $packages->insert_weights($code, $item_no, $weight[0], $weight[1], $weight[2], $weight[3], $weight[4], $weight[5], $weight[6], $weight[7], $weight[8], $weight[9]); 
		}
	} 
