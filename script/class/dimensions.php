...
class dimensions {
	...
	public function insert($code, $item_no, $length, $width, $height) {
		...
		if ($result) {
			$this->output->info("Item: $item_no, Code: $code - Dimensions $length x $width x $height has been inserted successfully!");
			return true;
		} else {
	...
	public function update($code, $item_no, $length, $width, $height) {
		...
		if ($result) {
			$this->output->info("Item: $item_no, Code: $code - Dimensions has been updated to $length x $width x $height!");
			return true;
		} else {
	...
	public function get_dimensions($code, $item_no) {
		...
		if (mysqli_num_rows($result) > 0) {
			...
			$this->output->info("Item: $item_no, Code: $code - Dimensions $length x $width x $height found!");
		} else {
